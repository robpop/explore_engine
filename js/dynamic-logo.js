...
function intToRGB(t){let n=(16777215&t).toString(16).toUpperCase();return"00000".substring(0,6-n.length)+n}

function generate__m(logo, mobile_id, proper_logo_width) {
  let right = ((128 - $(".mobile-library-section").width())/2)-0.23;
  let mlogo = $(logo).clone(true, true);
  $(mlogo).attr("id", '');
  $(mlogo).attr("data-mgen", true);
  $(mlogo).css({
  ...
  $("[data-mobile]").each((idx, data) => {
    if($(data).hasClass("mobile-library-section") && !$(data).hasClass("-non-mobile") && $(data).attr("data-mobile") == mobile_id) {
      $(data).find(".mobile-library-preview").append(mlogo);
      $(data).find(".mobile-library-preview").addClass("mobile-library-preview__svg");
    }
  });
}

function generate(ele, url, proper_logo_width) {
  try {
    ...
    // set the corresponding mobile card
    if(typeof proper_logo_width !== "undefined") generate__m($(ele).find("svg"), $(ele).attr("data-mobile"), proper_logo_width);
